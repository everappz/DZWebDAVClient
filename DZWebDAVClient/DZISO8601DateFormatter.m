...
#endif
const unichar DZISO8601DefaultTimeSeparatorCharacter = DEFAULT_TIME_SEPARATOR;
...
static NSString * const DZISO8601TwoCharIntegerFormat = @"%.2d";

@interface DZISO8601DateFormatter(UnparsingPrivate)
...
@implementation DZISO8601DateFormatter
...
- (id) init {
	if ((self = [super init])) {
		...
		format = DZISO8601DateFormatCalendar;
		timeSeparator = DZISO8601DefaultTimeSeparatorCharacter;
		includeTime = NO;
		parsesStrictly = NO;
...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange fractionOfSecond:(out NSTimeInterval *)outFractionOfSecond {
	...
	if (strict) timeSep = DZISO8601DefaultTimeSeparatorCharacter;
...
- (NSString *) stringFromDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	switch (self.format) {
		case DZISO8601DateFormatCalendar:
			return [self stringFromDate:date formatString:ISO_CALENDAR_DATE_FORMAT timeZone:timeZone];
		case DZISO8601DateFormatWeek:
			return [self weekDateStringForDate:date timeZone:timeZone];
		case DZISO8601DateFormatOrdinal:
			return [self stringFromDate:date formatString:ISO_ORDINAL_DATE_FORMAT timeZone:timeZone];
		default:
			[NSException raise:NSInternalInconsistencyException format:@"self.format was %tu, not calendar (%tu), week (%tu), or ordinal (%tu)", self.format, DZISO8601DateFormatCalendar, DZISO8601DateFormatWeek, DZISO8601DateFormatOrdinal];
			return nil;
	}
}
...
- (NSString *) stringFromDate:(NSDate *)date formatString:(NSString *)dateFormat timeZone:(NSTimeZone *)timeZone {
	...
	if (includeTime) {
		...
		if (offset == 0)
            ...
            str = [str stringByAppendingFormat:DZISO8601TwoCharIntegerFormat, timeZoneOffsetHour];
            
            if (self.timeZoneSeparator) str = [str stringByAppendingFormat:@"%C", self.timeZoneSeparator];
            
            str = [str stringByAppendingFormat:DZISO8601TwoCharIntegerFormat, timeZoneOffsetMinute];
		}
	}
...
- (NSString *) weekDateStringForDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	...
	if(includeTime) {
		...
		unichar timeSep = self.timeSeparator;
		if (!timeSep) timeSep = DZISO8601DefaultTimeSeparatorCharacter;
